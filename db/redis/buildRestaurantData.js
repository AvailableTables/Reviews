//cat redisRest.csv | redis-cli --pipe
...
var stream = fs.createWriteStream('redisRest.csv');
...
function generate ()  {
  ...
  while(x < amount){
    ...
    while(count<50){
      ...
      chunk +=  `HMSET restaurants ${x++} "${model.addRestaurant(buildData.restaurantDataMaker())}"\n`;
      count+=1;
    }
    count=0
